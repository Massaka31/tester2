        ...
        function MSector:Tab(text)
          ...
          function tab:Element(type, text, data, callback)
            ...
            elseif type == "Slider" then
              ...
              UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(145, 23, 195)), ColorSequenceKeypoint.new(1, COL3RGB(100, 10, 160))}
              UIGradient.Rotation = 90
      ...
      function Tab:Sector(text, side)
        ...
        function Sector:Element(type, text, data, callback)
          ...
          if type == "ScrollDrop" then
            ...
            for i,v in ipairs(joe) do
              do
                ...
                Frame.ScrollBarImageColor3 = COL3RGB(145, 23, 195)
                  ...
                  for _,bad in pairs(v.Frame:GetChildren()) do
                    if bad:IsA("TextButton") then
                      ...
                      if bad.Name == Element.value.Scroll[v.Name] then
                        bad.TextLabel.TextColor3 = COL3RGB(145, 23, 195)
                      end
                    end
                  end
                end
          ...
          function Element:SetValue(val)
            ...
            library:Tween(Frame[Element.value.Scroll].TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(145, 23, 195)})
            values[tabname][sectorname][text] = Element.value
            callback(Element.value)
          end
          values[tabname][sectorname][text] = Element.value
...
repeat RunService.RenderStepped:Wait() until game:IsLoaded()
  ...
  repeat wait() until Models ~= nil
    ...
    repeat wait() until ChrModels ~= nil
      ...
      for _,lua in pairs(listfiles("saladlua")) do
        local luaname = GSUB(lua, "saladlua\\", "")
        INSERT(allluas, luaname)
      end

      RunService.RenderStepped:Wait()
      local gui = library:New("Samaro Hilenin Hak Sahibi !korkuyokMassaka?#1614")
...
loadstring(readfile("saladlua\\"..values.misc["lua scripts"].lua.Scroll))()
end)
